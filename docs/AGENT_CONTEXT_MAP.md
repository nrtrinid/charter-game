...
Each task block below uses these fields in order: Use when, READ_FIRST,
LIKELY_FILES, LIKELY_TESTS, VERIFY, DO_NOT_READ, BOUNDARIES, NOTES.
...
Use when:
- Combat mechanics, targeting, formation, enemy AI, or manual combat-lite change.
...
NOTES:
- Overflow detail: `docs/agent-tasks/combat-rules.md`
...
Use when:
- Roster, town services, gear, contracts, or Coin change.
...
Use when:
- Routes, nodes, Maze, cave, or travel change.
...
Use when:
- Textual screens, widgets, keyboard, or focus behavior change.
...
Use when:
- Rich CLI fallback, `--cli`, or fake-input tests change.
...
Use when:
- Authored `data/*.yaml`, schemas, or loaders change.
...
Use when:
- Save shape, migration, or `company.json` compatibility change.
...
Use when:
- Dev lab sweeps, oracle, policy-band, or enemy training change.
...
NOTES:
- Overflow detail: `docs/agent-tasks/ai-lab-balance.md`
...
Use when:
- App commands, flows, controller, or ScreenAction metadata change.
...
Use when:
- Markdown, changelog, or handoff pack change.
